...
{
    // Not sure why this doesn't work, but it would be better than copying
    // all of "update" below from dynamicRefineFvMesh
    //
    //dynamicRefineFvMesh& dRFM = dynamic_cast<dynamicRefineFvMesh&>(*this);
    //bool hasChanged = dRFM.update();
    ...
    if ( time().timeIndex() > 0 && Pstream::parRun() && hasChanged )
    {
        ...
        label nGlobalCells = nCells();
        Foam::reduce(nGlobalCells, sumOp<label>());
